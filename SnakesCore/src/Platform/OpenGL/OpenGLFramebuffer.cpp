...
namespace sk {
	...
	void OpenGLFramebuffer::Invalidate()
	{
		...
		{
			SK_ASSERT(m_ColorAttachments.size() > 4, "");
		...
		SK_ASSERT(glCheckFramebufferStatus(GL_FRAMEBUFFER) != GL_FRAMEBUFFER_COMPLETE, "Framebuffer is incomplete!");
	...
	void OpenGLFramebuffer::Resize(int width, int height)
	{
		if (width <= 0 || height <= 0)
		{
			SK_ERROR("Trying to resize the framebuffer to an invalid values: {0}, {1}", width, height);
			return;
		}
	...
	{
		SK_ASSERT(attachmentIndex >= m_ColorAttachments.size(), "");
