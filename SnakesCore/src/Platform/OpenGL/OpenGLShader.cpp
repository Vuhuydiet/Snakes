...
namespace sk {
	...
	static GLenum StringToGLShaderType(const std::string& type)
	{
		...
		SK_ASSERT(true, "Unknown shader type!");
		return 0;
	}
	...
	std::string OpenGLShader::ReadFile(const std::string& filepath)
	{
		...
		if (!in)
		{
			SK_ERROR("Could not open file '{0}'!", filepath);
		}
	...
	std::unordered_map<GLenum, std::string> OpenGLShader::PreProcess(const std::string& src)
	{
		...
		while(pos != std::string::npos)
		{
			...
			SK_ASSERT(!StringToGLShaderType(type), "invalid shader type!");
	...
	void OpenGLShader::Compile(const std::unordered_map<GLenum, std::string>& shaderSrcs)
	{
		...
		for (const auto& [type, src] : shaderSrcs)
		{
			...
			if (isCompiled == GL_FALSE)
			{
				...
				SK_ERROR("{0}", infoLog.data());
				SK_ASSERT(false, "Shader compilation failed!");
				return;
			}
		...
		if (isLinked == GL_FALSE)
		{
			...
			SK_ERROR("{0}", infoLog.data());
			SK_ASSERT(false, "Shader program linking error!");
			return;
		}
	...
	inline uint32_t OpenGLShader::GetLocation(const std::string& type, const std::string& name)
	{
		...
		if (location < 0)
		{
			SK_ERROR("No such '{0}' uniform named '{1}' in shader '{2}', misspelled or was not be used?", type, name, m_Name);
		}
		return location;
	}
