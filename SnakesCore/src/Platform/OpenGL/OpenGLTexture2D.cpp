...
namespace sk {
		...
		: m_FilePath(filepath), m_Width(0), m_Height(0), m_RendererID(0), m_InternalFormat(0), m_DataFormat(0)
	{
		...
		{
			m_Valid = false;
			SK_ERROR("Failed to load image: '{0}'!", filepath.string());
			return;
		}
		...
		{
			m_Valid = false;
			SK_ERROR("Format not supported: '{0}'!", filepath.string());
			return;
		}
	...
	void OpenGLTexture2D::SetData(void* data, size_t size)
	{
		...
		SK_ASSERT(size != m_Width * m_Height * bpp, "The data set to the texture must be the same size!");
		glTextureSubImage2D(m_RendererID, 0, 0, 0, m_Width, m_Height, m_DataFormat, GL_UNSIGNED_BYTE, data);
	}
